...
export class PowerAppsConnection extends DeserializeJSON implements IPowerAppsConnection {
    ...
    async refreshToken(config: IAuthParams): Promise<string> {
        ...
        // using https://microsoftedge.microsoft.com/addons/detail/cors-unblock/hkjklmhkbkdhlgnnfbbcihcajofmjgbh
        // For testing without worrying about CORS
    ...
    public static async authenticate(config: IAuthParams): Promise<PowerAppsConnection> {
        ...
        switch (config.grant_type) {
            ...
            case GrantType.PASSWORD:
                urlencoded.append("username", config.username as string);
                urlencoded.append("password", config.password as string);
